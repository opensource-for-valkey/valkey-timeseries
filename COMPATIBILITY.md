...
**Security Architecture.** Valkey TimeSeries's security architecture is somewhat more restrictive than RedisTimeSeries. Valkey TimeSeries enforces a user's ACL keyspace restrictions on label-based, multi-series query operations (`TS.MRANGE`, `TS.MREVRANGE`, `TS.MGET`, `TS.QUERYINDEX`, `TS.QUERYLABELS`, and the label-discovery commands), so a client only sees series whose keys it is permitted to access; RedisTimeSeries applies these restrictions less consistently. `TS.QUERYLABELS` is deliberate about this: unlike `TS.QUERYINDEX`, which reveals every matching key regardless of read access, it silently omits unreadable series so that label names/values belonging only to them never appear in the result.
...
The set of commands exposed by the module, their argument ordering, their flag and option names, and the shape of their replies are expected to match RedisTimeSeries. This includes series-management commands (e.g. `TS.CREATE`, `TS.ALTER`, `TS.DEL`), the compaction-rule commands (`TS.CREATERULE`, `TS.DELETERULE`), ingestion commands (`TS.ADD`, `TS.MADD`, `TS.INCRBY`, `TS.DECRBY`), and query commands (`TS.GET`, `TS.MGET`, `TS.RANGE`, `TS.REVRANGE`, `TS.READ`, `TS.MRANGE`, `TS.MREVRANGE`, `TS.NRANGE`, `TS.NREVRANGE`, `TS.INFO`, `TS.QUERYINDEX`, `TS.QUERYLABELS`). Applications using standard RedisTimeSeries client libraries should be able to issue these commands unmodified and receive replies they can parse with existing response handlers.

`TS.READ` carries one deliberate metadata difference from RedisTimeSeries 8.10: its key specification declares `RO` **and** `ACCESS`, where the reference declares `RO` alone. `ACCESS` is the semantically correct flag for a command that returns key data to the caller, it matches every other read command in this module, and dropping it would weaken ACL key-permission checking. The difference is not observable in the reply to any command.
...
| `TS.DEL` count over an already-expired range (DIV-0021) | RedisTimeSeries counts expired samples still physically stored, which depends on its chunk-granular trim schedule. Valkey TimeSeries trims on a different schedule and drops the expired prefix before a delete, so it reports a deterministic `0`; every read path, `TS.INFO` included, agrees on both engines. |
...
| `EMPTY` fill on a bucket grid that extends below timestamp 0 — an `ALIGN` offset that is not a multiple of the bucket duration, with data within one bucket of the epoch (DIV-0054) | RedisTimeSeries suppresses the leading and trailing empty-bucket fill in that state only, and disagrees with itself doing so: its own `TS.REVRANGE` returns the bucket its `TS.RANGE` drops, and its `TS.RANGE` returns that bucket as soon as it holds a sample. Matching means dropping in-range buckets to reproduce an inconsistency; every non-negative grid agrees exactly, boundary buckets included. |
