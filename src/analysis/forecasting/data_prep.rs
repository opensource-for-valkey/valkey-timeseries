...
impl TimeSeries {
    ...
    pub fn new(
        ...
        for i in 1..timestamps.len() {
            ...
            delta.insert(timestamps[i], sample);
        }
        Ok(Self {
            timestamps,
            values,
    ...
    pub fn sanitized(&mut self, policy: MissingValuePolicy) -> Result<(), ForecastError> {
        match policy {
            ...
            MissingValuePolicy::Drop => {
                ...
                let values: Vec<f64> = valid_indices.iter().map(|&i| self.values[i]).collect();
            }
    ...
    pub fn interpolated(&self, fill_edges: bool) -> TimeSeries {
        let values = interpolate_series(&self.values, fill_edges);
    ...
    pub fn imputed_forward_backward(&mut self) -> TimeSeries {
        // Forward fill
    ...
    pub fn imputed_seasonal(&self, period: usize) -> Result<(), ForecastError> {
        if period == 0 {
            return Err(ForecastError::InvalidParameter(
    ...
    pub fn with_outliers_replaced(
        ...
        let vals = self.values();
        let outlier_result = detect_outliers(vals, config);
    ...
    pub fn sliding_window_aggregate(
        ...
        let values: Vec<f64> = starts
            .iter()
            .map(|&s| aggregate_slice(&self.values[s..s + window], method))
            .collect();
        TimeSeries {
            timestamps,
            values,
...
mod tests {
    ...
    fn fill_gaps_with_monthly_frequency() {
        ...
        assert!(filled.values()[1].is_nan()); // Feb is filled with NaN
    }
    ...
    fn agg_sum_exact() {
        ...
        assert_eq!(a.len(), 2);
        assert_relative_eq!(a.values()[0], 6.0);
        assert_relative_eq!(a.values()[1], 15.0);
    }
    ...
    fn agg_sum_rem() {
        ...
        let a = ts.aggregate(3, AggregationMethod::Sum);
        assert_eq!(a.len(), 3);
        assert_relative_eq!(a.values()[2], 7.0);
    }
    ...
    fn agg_mean_exact() {
        ...
        assert_eq!(a.len(), 3);
        assert_relative_eq!(a.values()[0], 3.0);
        assert_relative_eq!(a.values()[1], 7.0);
        assert_relative_eq!(a.values()[2], 11.0);
    }
    ...
    fn agg_mean_rem() {
        ...
        assert_relative_eq!(a.values()[0], 20.0);
        assert_relative_eq!(a.values()[1], 45.0);
    }
    ...
    fn agg_median_test() {
        ...
        assert_relative_eq!(a.values()[0], 3.0);
        assert_relative_eq!(a.values()[1], 4.5);
    }
    ...
    fn agg_first_last_min_max() {
        ...
        assert_relative_eq!(
            ts.aggregate(2, AggregationMethod::First).values()[0],
            3.0
        );
        assert_relative_eq!(
            ts.aggregate(2, AggregationMethod::Last).values()[0],
            1.0
        );
        assert_relative_eq!(
            ts.aggregate(2, AggregationMethod::Min).values()[0],
            1.0
        );
        assert_relative_eq!(
            ts.aggregate(2, AggregationMethod::Max).values()[0],
            3.0
        );
    }
    ...
    fn sw_sum_s1() {
        ...
        assert_relative_eq!(s.values()[1], 9.0);
    ...
    fn sw_mean_s2() {
        ...
        assert_eq!(s.len(), 2);
        assert_relative_eq!(s.values()[0], 4.0);
        assert_relative_eq!(s.values()[1], 8.0);
    }
    ...
    fn sw_edge() {
        ...
        assert_relative_eq!(s.values()[2], 7.0);
        assert!(TimeSeries::univariate(vec![], vec![])
            .unwrap()
...
mod serde_tests {
    ...
    fn with_outliers_replaced_no_change_when_clean() {
        ...
        assert_eq!(clean.values(), ts.values());
    }
    ...
    fn with_outliers_replaced_multiple_outliers() {
        ...
        assert!((clean.values()[10] - 10.0).abs() < 1.0);
        assert!((clean.values()[50] - 10.0).abs() < 1.0);
        assert!((clean.values()[90] - 10.0).abs() < 1.0);
    }
