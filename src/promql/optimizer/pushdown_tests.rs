...
mod tests {
    ...
    #[case(r#"foo{x="y"} * on(bar) baz{a="b"}"#, r#"foo{x="y"} * on (bar) baz{a="b"}"#)]
    #[case(
    ...
    #[case(r#"{x="y",__name__="a"} + {a="b"}"#, r#"a{a="b", x="y"} + {a="b", x="y"}"#)]
    #[case(
    ...
    #[case(r#"foo{a="b"} unless bar{c="d"}"#, r#"foo{a="b"} unless bar{a="b", c="d"}"#)]
    #[case(r#"foo{a="b"} unless on() bar{c="d"}"#, r#"foo{a="b"} unless on () bar{c="d"}"#)]
    #[case(
    ...
    #[rstest]
    #[case(r#"sum(foo{bar="baz"}) / a{b="c"}"#, r#"sum(foo{bar="baz"}) / a{b="c"}"#)]
    #[case(r#"sum(foo{bar="baz"}) by () / a{b="c"}"#, r#"sum(foo{bar="baz"}) by () / a{b="c"}"#)]
    #[case(
    ...
    #[case(r#"sum(foo{bar="baz"}) by (x) / a{b="c"}"#, r#"sum(foo{bar="baz"}) by (x) / a{b="c"}"#)]
    #[case(
    ...
    #[rstest]
    #[case(r#"(foo @ end()) + bar{baz="a"}"#, r#"(foo{baz="a"} @ end()) + bar{baz="a"}"#)]
    #[case(r#"sum(foo @ end()) + bar{baz="a"}"#, r#"sum(foo @ end()) + bar{baz="a"}"#)]
