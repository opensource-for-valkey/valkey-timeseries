...
impl Parser {
    ...
    fn parse(mut self, input: &str) -> Result<Vec<Series>, String> {
    ...
    fn parse_sample_line(&mut self, line: &str) -> Result<(), String> {
        ...
        self.series_map
            .entry(fingerprint)
            .and_modify(|series| series.samples.push(sample))
    ...
    fn parse_metric_name_and_labels<'a>(
        &self,
        line: &'a str,
    ) -> Result<(String, Vec<Label>, &'a str), String> {
    ...
    fn parse_value_and_timestamp(&self, rest: &str) -> Result<(f64, i64), String> {
...
fn find_closing_brace(s: &str, start: usize) -> Result<usize, String> {
...
fn parse_labels(s: &str) -> Result<Vec<Label>, String> {
    if s.is_empty() {
        return Ok(Vec::new());
    }
...
/// ```
pub(crate) fn parse_openmetrics(input: &str) -> Result<Vec<Series>, String> {
...
mod tests {
    ...
    #[case::counter("counter", "test_total 1\n# EOF",
        MetricType::Sum{ monotonic: true, temporality: Temporality::Cumulative }
    )]
    ...
    #[case::histogram_bucket("histogram", "test_bucket{le=\"1\"} 10\n# EOF", "_bucket",
        MetricType::Histogram{ temporality: Temporality::Cumulative }
    )]
    ...
    fn should_parse_timestamps(#[case] ts_str: &str, #[case] expected_ms: i64) {
        ...
        // then
        assert_eq!(samples[0].samples[0].timestamp, expected_ms);
    }
    ...
    fn should_use_current_time_when_timestamp_missing() {
        ...
        let after = SystemTime::now()
            ...
            .unwrap()
            .as_millis() as i64;
        assert!(samples[0].samples[0].timestamp >= before);
        assert!(samples[0].samples[0].timestamp <= after);
    }
