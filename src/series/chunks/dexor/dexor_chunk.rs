//! `DeXorChunk`: a timeseries chunk whose values are DeXOR-coded and whose
//! timestamps use Gorilla's delta-of-delta varbit scheme.
//!
//! Structurally a twin of [`GorillaChunk`](crate::series::chunks::GorillaChunk)
...
use super::dexor_encoder::DeXorEncoder;
...
pub struct DeXorChunk {
    pub(crate) encoder: DeXorEncoder,
    pub max_size: usize,
}

impl Default for DeXorChunk {
    fn default() -> Self {
...
impl DeXorChunk {
    pub fn with_max_size(max_size: usize) -> Self {
        Self {
            encoder: DeXorEncoder::new(),
            max_size,
        }
    }

    fn compress(&mut self, samples: &[Sample]) -> TsdbResult {
        let mut encoder = DeXorEncoder::new();
    ...
    pub fn range_iter(&'_ self, start_ts: Timestamp, end_ts: Timestamp) -> SampleIter<'_> {
        DeXorChunkIterator::new(self, start_ts, end_ts).into()
    }
}

impl ChunkOps for DeXorChunk {
    ...
    fn remove_range(&mut self, start_ts: Timestamp, end_ts: Timestamp) -> TsdbResult<usize> {
        ...
        let mut new_encoder = DeXorEncoder::new();
    ...
    fn is_full(&self) -> bool {
        self.encoder.buf().len() >= self.max_size
    }
...
impl Chunk for DeXorChunk {
    fn split(&mut self) -> TsdbResult<Self> {
        if self.is_empty() {
            return Ok(self.clone());
        }

        let mut left = DeXorEncoder::new();
        let mut right = DeXorChunk::with_max_size(self.max_size);
    ...
    fn load_rdb(rdb: *mut RedisModuleIO, _enc_ver: i32) -> ValkeyResult<Self> {
        ...
        let encoder = DeXorEncoder::rdb_load(rdb)?;
        Ok(DeXorChunk { encoder, max_size })
    }
    ...
    fn deserialize(buf: &[u8]) -> TsdbResult<Self> {
        ...
        let encoder = DeXorEncoder::deserialize(buf)?;
        Ok(DeXorChunk {
            encoder,
...
#[inline]
fn push_sample(encoder: &mut DeXorEncoder, sample: &Sample) -> TsdbResult {
    encoder.add_sample(sample).map_err(|e| {
        log_warning(format!("dexor: error adding sample: {e:?}"));
        TsdbError::CannotAddSample(*sample)
    })
}

pub struct DeXorChunkIterator<'a> {
    inner: DeXorIterator<'a>,
    start: Timestamp,
    end: Timestamp,
    init: bool,
}

impl<'a> DeXorChunkIterator<'a> {
    pub fn new(chunk: &'a DeXorChunk, start: Timestamp, end: Timestamp) -> Self {
        Self {
            inner: DeXorIterator::new(&chunk.encoder),
            start,
            end,
            init: false,
        }
    ...
    fn next_internal(&mut self) -> Option<Sample> {
        match self.inner.next() {
            ...
            Some(Err(err)) => {
                log_warning(format!("dexor: error decoding sample: {err:?}"));
                None
            }
...
impl Iterator for DeXorChunkIterator<'_> {
    type Item = Sample;
