...
fn decompress(chunk: &DeXorChunk) -> Vec<Sample> {
    chunk.iter().collect()
}

fn filled_chunk(count: usize) -> (DeXorChunk, Vec<Sample>) {
    let samples = generate_samples(count);
    let mut chunk = DeXorChunk::with_max_size(16384);
...
fn test_clear() {
    ...
    // A cleared chunk must be usable again, which means the value codec's
    // predictor state was reset along with the bit stream.
...
fn test_upsert() {
    for chunk_size in (64..8192).step_by(512) {
        ...
        let mut chunk = DeXorChunk::with_max_size(chunk_size);
...
#[test]
fn test_iter() {
    let mut chunk = DeXorChunk::default();
...
fn test_serialize_roundtrip() {
    ...
    let restored = DeXorChunk::deserialize(&buf).unwrap();

    assert_eq!(restored, chunk);
    assert_eq!(decompress(&restored), decompress(&chunk));
}

/// The value codec carries predictor state (`previous_value`, `q`, `delta`, and
/// the exception coder's adaptive field width) across samples. If any of it is
/// lost on the way through `serialize`, samples appended afterwards decode as
/// garbage — so a restored chunk must produce the same stream as one that was
/// never persisted.
#[test]
fn test_append_after_serialize_roundtrip() {
    for prefix_len in [1usize, 2, 3, 17, 200] {
        ...
        let mut chunk = DeXorChunk::with_max_size(65536);
        ...
        let mut buf = Vec::new();
        chunk.serialize(&mut buf);
        let mut restored = DeXorChunk::deserialize(&buf).unwrap();
...
/// Same contract as above, but exercised after a run of exceptions so the
/// exception coder's `EL` width is away from its initial value when the
/// snapshot is taken.
#[test]
fn test_append_after_serialize_roundtrip_with_exceptions() {
    let mut samples = Vec::new();
    for i in 0..120i64 {
        // Alternate decimal-friendly values with binary-origin ones that force
        // the exception path.
        let value = if i % 2 == 0 {
            1.5 + (i % 10) as f64 * 0.1
        } else {
            f64::from_bits(0x4000_0000_0000_0001 + i as u64 * 0x0003_1000_0000_0000)
        };
        samples.push(Sample {
...
fn test_deserialize_rejects_corrupt_codec_state() {
    ...
    for cut in 0..buf.len() {
        let _ = DeXorChunk::deserialize(&buf[..cut]);
    }
    ...
    for i in 0..buf.len().min(40) {
        ...
        let _ = DeXorChunk::deserialize(&corrupt);
    }
}
...
fn test_single_and_empty_chunk() {
    let empty = DeXorChunk::default();
    assert!(empty.is_empty());
    assert!(decompress(&empty).is_empty());

    let mut one = DeXorChunk::default();
    let sample = Sample {
        timestamp: 1234,
        value: 21.5,
    };
...
/// DeXOR exists to beat binary XOR coders on decimal-origin data. If it stops
/// doing that on a three-decimal-digit series, something has regressed.
#[test]
fn test_beats_gorilla_on_decimal_data() {
    let samples = generate_samples(4096);

    let mut dexor = DeXorChunk::with_max_size(1 << 20);
    let mut gorilla = GorillaChunk::with_max_size(1 << 20);
    for sample in &samples {
        dexor.add_sample(sample).unwrap();
        gorilla.add_sample(sample).unwrap();
    }

    let dexor_bytes = dexor.encoder.buf().len();
    let gorilla_bytes = gorilla.encoder.buf().len();
    assert!(
        dexor_bytes < gorilla_bytes,
        "dexor used {dexor_bytes} bytes vs gorilla's {gorilla_bytes} on 3-decimal-digit data",
    );
}
