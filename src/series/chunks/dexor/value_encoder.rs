...
//! [`DeXorMode`](super::DeXorMode) rather than a subclass hierarchy.
//!
//! The encoder does not own its output. It writes into a caller-supplied
//! [`BitStream`] so that a chunk can interleave timestamps between values.
...
use super::{
    CONTROL_BITS, CONTROL_EXCEPTION, CONTROL_NEW_DELTA, CONTROL_NEW_Q, CONTROL_REPEAT,
    CodecSnapshot, DELTA_BITS, DeXorConfig, MAX_DELTA, Q_BIAS, Q_BITS, Q_MAX, Q_MIN, SharedState,
};
...
pub struct DeXorValueEncoder {
    exceptions: ExponentCoder,
    state: SharedState,
}

impl Default for DeXorValueEncoder {
    fn default() -> Self {
        Self::new(DeXorConfig::default())
    }
}

impl Hash for DeXorValueEncoder {
    fn hash<H: Hasher>(&self, hasher: &mut H) {
        self.exceptions.hash(hasher);
        self.state.hash(hasher);
    }
}

impl DeXorValueEncoder {
    ...
    pub fn config(&self) -> DeXorConfig {
        self.state.config
    }

    pub fn reset(&mut self) {
        *self = Self::new(self.state.config);
    }

    /// Capture the predictor state so an encoder can resume appending to a
    /// stream after a persistence round-trip.
    ///
    /// Only [`DeXorMode::Native`] state is captured; see [`CodecSnapshot`].
    ///
    /// [`DeXorMode::Native`]: super::DeXorMode::Native
    pub(super) fn snapshot(&self) -> CodecSnapshot {
        debug_assert!(self.state.window.is_empty() && !self.state.skipping);
        let (previous_exp, exponent_len, contract_step) = self.exceptions.snapshot();
        CodecSnapshot {
            previous_value_bits: self.state.previous_value.to_bits(),
            previous_q: self.state.previous_q,
            previous_delta: self.state.previous_delta,
            previous_exp,
            exponent_len,
            contract_step,
        }
    ...
    /// Rebuild a `Native`-mode encoder from a snapshot.
    ///
    /// Returns `None` if the snapshot is out of range — it comes from RDB or a
    /// wire buffer, so it is untrusted.
    pub(super) fn restore(config: DeXorConfig, snapshot: CodecSnapshot) -> Option<Self> {
        if !(Q_MIN..=Q_MAX).contains(&snapshot.previous_q) || snapshot.previous_delta >= MAX_DELTA {
            return None;
        }
        let exceptions = ExponentCoder::restore(
            config.rho,
            snapshot.previous_exp,
            snapshot.exponent_len,
            snapshot.contract_step,
        )?;

        let mut state = SharedState::new(config);
        state.previous_value = f64::from_bits(snapshot.previous_value_bits);
        state.previous_q = snapshot.previous_q;
        state.previous_delta = snapshot.previous_delta;

        Some(Self { exceptions, state })
    }

    /// Append one value to `out`.
    pub fn encode(&mut self, out: &mut BitStream, value: f64) {
        if self.state.skipping {
            self.exceptions.encode(out, value);
            return;
        }

        match self.plan(value) {
            Some(plan) => self.write_decimal(out, value, plan),
            None => {
                write_bits(out, CONTROL_BITS, CONTROL_EXCEPTION);
                self.exceptions.encode(out, value);
                self.state.reject();
            }
        }
    ...
    fn write_decimal(&mut self, out: &mut BitStream, value: f64, plan: Plan) {
        ...
        write_bits(out, CONTROL_BITS, control);
        write_bits(out, self.state.config.slot_bits(), plan.slot as u64);

        if !repeat {
            if !same_q {
                write_bits(out, Q_BITS, (plan.q + Q_BIAS) as u64);
                self.state.previous_q = plan.q;
            }
            write_bits(out, DELTA_BITS, u64::from(plan.delta));
            self.state.previous_delta = plan.delta;
        }

        // The prefix carries the sign unless it is zero.
        if approx_zero(plan.alpha) {
            out.write_bit(value > 0.0);
        }

        write_bits(out, decimal_bits(plan.delta), plan.beta);
