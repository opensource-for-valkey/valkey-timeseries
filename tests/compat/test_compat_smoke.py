...
class TestInfoBaseline:
    """Validates the frozen RTS 8.6 TS.INFO baseline (plan §5.1 rule 3).
    ...
    BASELINE_PATH = os.path.join(os.path.dirname(__file__), "info-fields-8.6.yml")
    ...
    def test_reference_matches_frozen_baseline(self, diff):
        ...
        assert actual == expected, (
            "RTS 8.6 TS.INFO field set drifted from tests/compat/info-fields-8.6.yml.\n"
    ...
    def test_subject_covers_baseline(self, diff):
        ...
        assert not missing, (
            f"subject TS.INFO is missing frozen RTS 8.6 fields: {missing}"
        )
